    ...
    const fetchDialogues = async () => {
        try {
            const token = localStorage.getItem("token");
                const response = await fetch("http://localhost:3001/home/api/dialogues", {
                method: "GET", 
                headers: {"token": `${token}`}
            });
...
// Выбираем диалог
    const handleClickDialogue = async (dialogue) => {
        try {
            ...
            const token = localStorage.getItem("token");
            const response = await fetch(`http://localhost:3001/api/set/messages/is/readed/${dialogue.group_id}`, {
                method: "POST",
                headers: {"token": `${token}`},
            });
            if (!response.ok) {
