...
const Settings = ({showSettings, closeSettings})=>{
    ...
    useEffect(()=>{
        try {
            ...
            const getProfileData = async function(){
                const response = await fetch((`http://localhost:3001/api/get/self/profile`),{
                    method: "GET",
                    headers: {
                        "token": `${token}`,
                    }
    ...
    const checkNickname = async function(nickname) {
        try {
            if (nickname.startsWith("@")) {
                ...
                const token = localStorage.getItem("token");
                const response = await fetch(`http://localhost:3001/api/check/nickname/is/valid/${nickname}`, {
                    headers: {"token": `${token}`},
                });
                if (!response.ok) {
        ...
        try {
            if (inavlidNicknameError.length === 0 && inavlidUsernameError.length === 0) {
                ...
                const setProfileData = async function(){
                    const response = await fetch((`http://localhost:3001/api/update/self/profile/${updatedFields}`),{
                        method: "POST",
                        headers: {
                            "token": `${token}`,
                        }
    ...
    const deleteAccount = function(){
        try {
            const token = localStorage.getItem("token");
            const response = fetch("http://localhost:3001/api/delete/account", {
                method: "POST",
                headers: {"token": `${token}`},
            });
            if (response.ok) {
